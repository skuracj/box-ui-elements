...
import {BoxItemFilter} from "../../common/types/core";
...
class ContentPicker extends Component<Props, State> {
    ...
    isItemNameMatchingFilters(itemName: string, itemType: string, filters: Array<BoxItemFilter>, isRootFolder: boolean): boolean {
        ...
        filters.forEach(({action, applyTo, type, pattern}) => {
            ...
            if (applyTo.includes(itemType)) {
                if (
                    (type === 'prefix' && itemName.startsWith(pattern)) ||
                    (type === 'postfix' && itemName.endsWith(pattern)) ||
                    (type === 'exact_match' && itemName === pattern) ||
                    (type === 'regexp' && itemName.match(new RegExp(pattern, "gi")))
                ) {
